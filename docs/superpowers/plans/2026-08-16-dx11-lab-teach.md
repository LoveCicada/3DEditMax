...
- `DemoPlayer`: `void start(const TeachingState& from)`; `bool tick(float dtSec, TeachingState* io)` — originally 8 seconds lerp `camYawDeg` 0→90 and back. **Superseded:** tour all 8 tutorial steps (~2s each); HUD captions. See spec §14.
